  ...
  var request = https.request(opc, function(res) {
    ...
    res.on('end', function() {
      ...
      try {
        ...
        console.log('Error:', e)
        //self.error('Error reading the dowloaded JSON', 'download.error');
        //deferred.reject(e);
        return;
      }
...
autoupdater.on('update.extracted', function() {
  ...
  pm2.connect(function(err) {
    ...
    pm2.restart('all', function(err, apps) {
...
const updateChecker = function(){
  try {
    ...
    console.log('Error check for updates:', err)
  }
}

setInterval(function(){
  updateChecker();
}, 5*60*1000); // check for update every 5 minutes
updateChecker();
